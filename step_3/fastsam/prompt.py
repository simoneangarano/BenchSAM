    ...
    def _format_results(self, result, filter=0):
        ...
        for i in range(n):
            ...
            if torch.sum(mask) < filter:
                continue
            annotation['id'] = i
            annotation['segmentation'] = mask.cpu()
            annotation['bbox'] = result.boxes.data[i]
            annotation['score'] = float(result.boxes.conf[i].cpu().numpy())
            annotation['area'] = int(annotation['segmentation'].sum().cpu().numpy())
        ...
        for i, annotation in enumerate(masks):
            ...
            for i, point in enumerate(points):
                ...
                if mask[point[1], point[0]] == 1 and pointlabel[i] == 0:
                    onemask[mask] = 0
        onemask = onemask >= 1
        return onemask, get_score(scores, areas)
...
def get_score(scores, areas):
    w_scores = [s * a for s, a in zip(scores, areas)]
    return sum(w_scores) / sum(areas) if areas else 0
